...
public class BigInt {
    ...
    public static BigInt randomBigInt(BigInt min, BigInt max) {
        ...
        } while (res.compareTo(minBig) < 0 || res.compareTo(maxBig) > 0); // checks if higher than min or lower than
                                                                          // max. inclusive
    ...
    public boolean checkEven() {
        ...
        int lastNum = Character.getNumericValue(this.value.charAt(length - 1));
        if (lastNum % 2 == 0) {
            return true;
        }
        return false;
    ...
    public static boolean isPrime(BigInt n, int t) { // n >= 3, t(security parameter) >= 1
        ...
        for (int i = 0; i < t; i++) {
            ...
            if (!y.equal(new BigInt("1")) && !y.equal(n.subtract(new BigInt("1")))) { // if y != 1 and y != n-1
                ...
                while (j.lessThanEqual(s.subtract(new BigInt("1"))) // j <= s-1
    ...
    public static BigInt generatePrime(BigInt min, BigInt max) {
        BigInt val;
        do {
